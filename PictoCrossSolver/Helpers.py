...
class ChangeMinimizer:
    ...
    @staticmethod
    def minimize(puzzle: Puzzle, changes: List[PuzzleChange]) -> List[PuzzleChange]:
        """
        Checks the changes for potential duplicate changes and minimizes the changes
        to only what is necessary. For example, an already set mark will not be changed 
        and the PuzzleChange might be simply removed if invalid or it can be split in two 
        change set.

        This is necessary to prevent a strategy from becoming bloated by tracking
        mark status in change sets.
        """
        newChanges = []
        for change in changes:

            # Find the affected indexes of this change
            zone = puzzle.getZone(change.getZoneType(), change.getZoneIndex())
            marks = zone.getMarks()[change.getZoneSlice()]
            markStatuses = set((markIndex, mark.getStatus().value) for markIndex, mark in enumerate(marks))

            # Find the target
            targetStatuses = set((markIndex, change.getAction().value) for markIndex, mark in enumerate(marks) if markIndex >= change.getZoneSlice().start and markIndex < change.getZoneSlice().stop)

            # Find the differrence and intersection between the 2 sets,
            # if no difference and no intersection, return same change
            # if len(differences) == len(targetStatuses), return the same change
            # if no difference but intersection found, return no nothing as change affects already set things
            # if different found, adjust the slice to create multiple changes based on original change
            differentIndexes = set(item[0] for item in targetStatuses.difference(markStatuses))
            sameIndexes = set(item[0] for item in targetStatuses.intersection(markStatuses))
            if (len(differentIndexes) == 0 and len(sameIndexes) == 0) or (len(differentIndexes) == len(targetStatuses)):
                newChanges.append(change)
                continue
            elif len(differentIndexes) == 0 and len(sameIndexes) != 0:
                continue
            else:
                newRanges = []
                for index in differentIndexes:
                    if len(newRanges) == 0:
                        newRanges.append((index, index))
                    elif newRanges[-1][1] != index -1:
                        newRanges.append((index, index))
                    else:
                        newRanges[-1] = (newRanges[-1][0], index)
                for newRange in newRanges:
                    newChanges.append(PuzzleChange(change.getZoneType(), change.getZoneIndex(), slice(newRange[0], newRange[1] + 1), change.getAction()))
